...
Prometheus를 띄우면 내장 Web UI에서 PromQL을 실행할 수 있지만, 그 화면은 쿼리 하나를 확인하는 용도지 여러 지표를 나란히 보며 상태를 판단하는 용도가 아닙니다. Grafana는 그 자리를 채우는 시각화 계층입니다.

### 관련 글

1. [Kubernetes 리소스 모니터링 (1) - Prometheus]({% post_url 2024/11/2024-11-07-prometheus %})
2. [Kubernetes 리소스 모니터링 (2) - Grafana (현재 글)]({% post_url 2024/11/2024-11-08-grafana %})
...
> **TL;DR**  
> - Grafana는 Prometheus, InfluxDB, Elasticsearch 등 여러 데이터 소스를 하나의 대시보드에서 시각화하는 도구입니다.  
> - Helm으로 설치할 때 storage class를 지정하면 PV와 PVC가 함께 만들어져 대시보드 설정이 Pod 재시작에도 남습니다.  
> - 설치 직후 admin 비밀번호는 Secret에 들어 있으므로, 접속한 뒤 바로 변경해야 합니다.  
{: .prompt-info}
