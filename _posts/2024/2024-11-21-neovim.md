...
require("lazy").setup({
  ...
  -- LSP 설정
  {
    ...
    config = function()
      ...
      -- tfenv 우선 PATH + 캐시 환경 구성
      local function tfenv_env(base)
        local env = {}; for k, v in pairs(base or {}) do env[k] = v end
        local home = vim.loop.os_homedir()
        local sep = (vim.fn.has("win32") == 1 or vim.fn.has("win64") == 1) and ";" or ":"
        local current = env.PATH or os.getenv("PATH") or ""
        local paths = { home .. "/.tfenv/shims", home .. "/.tfenv/bin" }
        for i = #paths, 1, -1 do
          local p = paths[i]
          if vim.fn.isdirectory(p) == 1 then current = p .. sep .. current end
        end
        env.PATH = current
      ...
      if vim.fn.executable("terraform-ls") == 1 then
        lsp.terraformls.setup({
          ...
          settings = {
            terraform = {
              path = "terraform", -- tfenv shim 사용. 0.12/0.13/1.x 프로젝트별 자동 선택
            },
          },
        })
      ...
      cmp.setup({
        ...
        mapping = cmp.mapping.preset.insert({
          ...
          ["<Tab>"] = cmp.mapping(function(fallback)
            ...
            if has_ultisnips and vim.fn["UltiSnips#CanJumpForwards"]() == 1 then
          ...
          ["<S-Tab>"] = cmp.mapping(function(fallback)
            if cmp.visible then
              if cmp.visible() then cmp.select_prev_item() return end
            end
