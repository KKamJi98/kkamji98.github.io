...
require("lazy").setup({
  ...
  -- 파일 트리 (Leader 기반 키맵)
  {
    ...
    keys = {
      { "<leader>e",  "<cmd>NvimTreeToggle<CR>",   mode = "n", silent = true, desc = "Toggle NvimTree" },
      { "<leader>fe", "<cmd>NvimTreeFindFile<CR>", mode = "n", silent = true, desc = "Find Current File in NvimTree" },
      { "<leader>fo", function()
          local ok, view = pcall(require, "nvim-tree.view")
          if ok and view.is_visible() then
            require("nvim-tree.api").tree.focus()
          else
            vim.notify("NvimTree is not open.", vim.log.levels.INFO, { title = "NvimTree" })
          end
        end,
        mode = "n", silent = true, desc = "Focus NvimTree (if open)" },
      { "<leader>fr", "<cmd>NvimTreeRefresh<CR>",  mode = "n", silent = true, desc = "Refresh NvimTree" },
    },
    dependencies = { "nvim-tree/nvim-web-devicons" },
    config = function()
