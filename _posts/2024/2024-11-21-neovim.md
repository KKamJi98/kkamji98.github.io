...
-- 성능·UX 강화, 안전한 lazy 부트스트랩, 경고 제로 세팅
...
-- Python3 provider 자동 지정(UltiSnips 대비)
...
if not vim.loop.fs_stat(lazypath) then
  vim.fn.system({
    ...
    "--branch=stable " .. vim.fn.shellescape(lazypath) .. " >/dev/null 2>&1"
  })
end
  ...
  -- Mason LSP bridge
  {
    ...
    config = function()
      require("mason-lspconfig").setup({
        ...
        automatic_installation = false,  -- 부팅시 설치 시도 금지
      })
    end,
  },
      ...
      lsp.lua_ls.setup({
        ...
        settings = {
          Lua = {
            ...
            diagnostics = { globals = { "vim" } },  -- undefined global ‘vim’ 경고 제거
            workspace = {
              checkThirdParty = false,
              library = vim.api.nvim_get_runtime_file("", true), -- Neovim API 인식
            },
