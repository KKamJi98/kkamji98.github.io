...
{% include diagrams/static/aws/irsa-web-identity-flow.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/aws/irsa-web-identity-flow--a335e2390100126a.png" %}
...
> **TL;DR**  
> - Pod 안의 애플리케이션이 S3, DynamoDB, Secrets Manager, Bedrock에 접근하지 못하는 문제를 IRSA로 해결합니다.  
> - IRSA는 EKS 클러스터의 OIDC provider를 신뢰 관계에 걸어 Kubernetes ServiceAccount에 IAM Role을 연결하는 방식입니다. Node의 인스턴스 프로파일을 빌리지 않으므로 Pod 단위로 권한을 좁힐 수 있습니다.  
> - 연결이 실제로 걸렸는지는 Pod 안에서 `aws sts get-caller-identity`로 Role ARN을 확인하고, web identity token 파일이 마운트됐는지 함께 봅니다.  
{: .prompt-info}
...
2. pod 내부에 접속 후, aws-cli를 통해 실제 aws 리소스에 접근 가능한지 확인
    ...
    bash-4.2# aws secretsmanager list-secrets
    {
        "SecretList": [
            {
                "ARN": "arn:aws:secretsmanager:us-east-1:393035689023:secret:<BASE64_ENCRYPTION_SECRET>",
