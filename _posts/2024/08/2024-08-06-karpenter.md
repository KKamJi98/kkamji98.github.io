...
> - Cluster Autoscaler는 ASG의 Desired Capacity를 조정하는 방식이라 ASG에 정의된 인스턴스 타입에 묶이지만, Karpenter는 미배치 Pod의 요구사항을 보고 인스턴스 타입을 직접 고릅니다. 확장 속도와 비용 양쪽에서 이 차이가 선택 근거가 됐습니다.  
> - 버전마다 Helm chart 변수와 필요한 IAM 정책이 달라, 과거 블로그 글을 따라가면 최신 버전 구축에 맞지 않습니다.  
{: .prompt-info}
...
> 현재 가장 최신 버전인 **0.37.0** 버전을 설치하겠습니다.  
{: .prompt-info}
