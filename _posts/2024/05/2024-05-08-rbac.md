...
{% include diagrams/static/kubernetes/kubernetes-rbac-binding.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/kubernetes/kubernetes-rbac-binding--454735d5bc2316f9.png" %}
...
클러스터 관리자 kubeconfig를 팀원에게 그대로 넘기면 그 사람은 모든 namespace의 모든 리소스를 만질 수 있습니다. 특정 namespace 안에서 Pod만 다루면 되는 개발자에게는 과한 권한입니다. Role과 RoleBinding으로 그 범위를 좁히는 과정을 실습합니다.

> **TL;DR**  
> - `exam-master-backend` namespace 안에서만 Pod를 생성, 조회, 수정, 삭제할 수 있는 사용자를 만드는 과정을 다룹니다.  
> - Role은 namespace 범위이므로 `exam-master-backend-developer` Role을 RoleBinding으로 묶으면 다른 namespace에는 권한이 넘어가지 않습니다.  
> - 권한이 실제로 좁혀졌는지는 `kubectl auth can-i`로 대상 namespace와 다른 namespace 양쪽을 확인해야 합니다.  
{: .prompt-info}
...
LS0tLS1.............S0tLS0K # -> 복사 후 아래 인증서 request에 붙여 넣기
```
