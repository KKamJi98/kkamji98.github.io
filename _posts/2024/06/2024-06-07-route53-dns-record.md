...
**Hosted zone**은 domain과 subdomain의 DNS routing 정보를 담는 record의 컨테이너입니다. Route 53에는 인터넷에서 조회되는 **public hosted zone**과 연결한 Amazon VPC에서 조회되는 **private hosted zone**이 있습니다. 두 종류는 같은 이름을 가질 수 있어도 조회 범위가 다르므로 만들기 전에 목적을 정해야 합니다.

public hosted zone을 만들면 Route 53은 기본 NS(Name Server) record와 SOA(Start of Authority) record를 생성합니다. NS record에 있는 name server들이 그 hosted zone의 authoritative name server입니다. Route 53이 도메인 등록기관이 아닌 경우에는 등록기관 콘솔에서 해당 도메인의 name server를 이 NS 값으로 변경해야 public hosted zone이 인터넷에서 권한을 갖습니다.
...
**Alias record**는 DNS 표준 record type이 아니라 Route 53의 확장 기능입니다. CNAME과 비슷하게 선택한 AWS 리소스로 질의를 보낼 수 있으며, 지원되는 대상에는 zone apex에서도 Alias를 만들 수 있습니다. 외부 호스팅 서비스의 custom domain 연결에는 그 서비스가 요구하는 최신 A, AAAA, CNAME 또는 TXT 값을 해당 서비스의 공식 문서에서 확인해 사용합니다. 과거에 기록한 IP 주소를 복사하는 방식은 대상 서비스의 변경에 취약합니다.

---

## 3. 웹 서비스용 record 설정 순서

다음은 `example.com`과 `www.example.com`을 설정하는 일반적인 순서입니다. 값은 실제 서비스 제공자가 안내한 현재 값을 사용해야 합니다.
...
DNS 변경은 hosted zone에 저장된 즉시 모든 recursive resolver의 cache에서 바뀌는 것이 아닙니다. TTL이 만료되기 전에는 이전 응답이 남을 수 있으므로, 배포와 장애 대응에서는 현재 authoritative 응답과 resolver cache 영향을 구분해야 합니다.
...
검증할 때는 record가 존재하는지만 보지 말고, public/private hosted zone을 혼동하지 않았는지, registrar delegation이 맞는지, CNAME 제약을 지켰는지, HTTPS 인증서의 domain ownership 검증 record가 남아 있는지 함께 확인합니다.
