...
{% include diagrams/static/kubernetes/horizontal-pod-autoscaling.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/kubernetes/horizontal-pod-autoscaling--ee7904cd45b0d9bb.png" %}

공식문서 - <https://kubernetes.io/ko/docs/tasks/run-application/horizontal-pod-autoscale/>

Kubernetes의 Horizontal Pod Autoscaler (HPA)는 클러스터에서 실행 중인 애플리케이션의 스케일링을 자동화하는 메커니즘입니다. HPA는 CPU 사용률, 메모리 사용률 등 다양한 메트릭을 기반으로 필요에 따라 Pod 수를 자동으로 조정합니다. 이를 통해 애플리케이션의 부하를 효과적으로 관리하고 리소스를 최적화할 수 있습니다.

> **TL;DR**  
> - HPA는 Metrics Server가 채운 `metrics.k8s.io`를 주기적으로 조회해 replica 수를 조정하므로, Metrics Server가 없으면 target이 `<unknown>`으로 남습니다.  
> - 부하를 제거해도 replica가 즉시 줄지 않는 이유는 기본 5분의 scale down 안정화 기간 때문입니다.  
> - 이 값은 HPA의 `behavior.scaleDown.stabilizationWindowSeconds`로 조정할 수 있습니다.  
{: .prompt-info}
...
Kubernetes에서 HPA는 특정 시간마다 Metrics Server를 통해 메트릭을 확인하고 Pod를 자동으로 스케일링합니다.  
...
```bash
root@Zest ~# kubectl run -i --tty load-generator --rm --image=busybox:1.28 --restart=Never -- /bin/sh
...
- [kubernetes.io - horizontal pod autoscale](https://kubernetes.io/ko/docs/tasks/run-application/horizontal-pod-autoscale/)
