...
> WEB 서버는 nginx:latest 이미지를 사용했고, WAS 서버는 tomcat:latest 이미지를 사용했고 DB 서버는 arm64v8/mysql:latest 이미지를 사용했습니다. arm 기반 OS를 사용 중이기 때문에 기존 mysql:latest 이미지를 사용하면 동작하지 않습니다. 또한 실습에서 Service는 외부에서 접속하지 않아도 되기 때문에 ClusterIP Type으로 생성했습니다. [ Type을 지정하지 않으면 ClusterIP 타입으로 생성됨 ]  
{: .prompt-tip}
...
> DNS 포트와 프로토콜인 53번 포트로 나가는 UDP, TCP 프로토콜 반드시 명시해야 합니다.  
{: .prompt-tip}
...
네임스페이스를 생성할 때, 네임스페이스에 레이블이 자동으로 추가되지 않는다는 사실과 network policy를 생성 직후 pod에서 다른 pod에 DNS 이름으로 접근이 불가능하다는 문제를 직면했습니다.
...
> **핵심 정리**  
> - 이 글은 `Network Policy 개념, 실습`의 개념, 구성 흐름, 실습 결과를 한 번에 따라갈 수 있도록 정리한 글입니다.  
...
- [nginx.org - Home](https://nginx.org/)
- [nginx.com - Home](https://nginx.com/)
- [cncf.io - guide to kubernetes egress network policies](https://www.cncf.io/blog/2020/02/10/guide-to-kubernetes-egress-network-policies/)
