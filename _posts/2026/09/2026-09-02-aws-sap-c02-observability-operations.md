...
![메트릭 데이터포인트 수집에서 알람 상태와 action 실행까지의 판정 순서와 결측 처리가 개입하는 지점](/assets/img/sap-c02/cloudwatch-alarm-missing-data.webp)
...
![로그 그룹에서 subscription filter, metric filter, S3 export, Logs Insights 네 경로로 나가는 구조와 각 경로의 제약](/assets/img/sap-c02/cloudwatch-logs-delivery-paths.webp)
...
![Config recorder에서 rule 평가와 자동 교정까지의 경로, proactive 평가와 aggregator가 멈추는 지점](/assets/img/sap-c02/config-evaluation-remediation.webp)
...
![진입 서비스에서 결정된 sampling 여부가 하위 서비스로 전파되는 경로와 하위 rule이 적용되지 않는 지점](/assets/img/sap-c02/xray-parent-based-sampling.webp)
...
![CloudWatch 알람과 Config 위반과 Health 이벤트와 CloudTrail 기록이 EventBridge를 거쳐 Automation과 SNS와 OpsItem으로 이어지는 구조](/assets/img/sap-c02/ops-event-routing.webp)
...
![배포 방식별로 새 버전이 뜨는 위치와 그에 대응하는 롤백 동작, 실패 버전의 트래픽 노출 여부](/assets/img/sap-c02/deployment-rollback-paths.webp)
