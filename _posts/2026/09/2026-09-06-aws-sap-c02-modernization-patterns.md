...
![실행 시간과 상태 보유 여부가 서버리스 컴퓨트 타깃을 가르는 결정 흐름](/assets/img/sap-c02/modernization-compute-target-decision.webp)
...
![strangler fig이 라우트를 바꾸는 동안 모놀리스를 롤백 지점으로 남기는 구조](/assets/img/sap-c02/modernization-strangler-route-switch.webp)
...
![ECR 이미지를 공통 공급 경로로 두고 ECS와 EKS Fargate의 운영 경계를 비교하는 구조](/assets/img/sap-c02/modernization-container-hosting-boundaries.webp)
...
![관계형 유지와 액세스 패턴 재설계 및 OS 제어 요구가 데이터베이스 타깃을 가르는 구조](/assets/img/sap-c02/modernization-database-target-selection.webp)
...
![버퍼와 순서와 콘텐츠 라우팅 요구를 SQS SNS EventBridge Step Functions로 나누는 선택 경계](/assets/img/sap-c02/modernization-integration-choice-boundaries.webp)
