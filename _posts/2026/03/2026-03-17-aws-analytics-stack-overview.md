...
AWS는 이 "S3에 쌓인 데이터를 분석하는 일"을 **여러 서비스의 조합**으로 풉니다. Athena, Glue, S3 Tables, Lake Formation, Redshift가 각자 한 가지 역할을 맡고 맞물려 돌아가죠. 이 조합을 저는 **AWS 데이터 분석 스택**이라고 부르겠습니다.

이 글은 시리즈의 첫 편으로, **각 구성요소가 무엇이고 어떻게 맞물리는지 큰 그림**을 잡는 데 집중합니다. 개별 서비스의 깊은 이야기와, 실제로 권한을 다 줬는데도 쿼리가 안 되던 트러블슈팅은 다음 편들에서 하나씩 풀어갑니다.
...
다만 초기 데이터 레이크(S3 + Parquet/JSON 파일 더미)는 약점이 있었습니다. 파일을 직접 다루다 보니 **트랜잭션(ACID)이 없고**, 스키마 변경이 까다롭고, 잘못된 부분 수정(update/delete)이 어렵습니다. "그냥 읽기 전용 로그 분석"엔 충분해도, 정합성이 필요한 데이터엔 부족했죠.
...
> 이 "두 축" 때문에 실무에서 가장 흔한 함정이 생깁니다. **IAM 권한을 완벽하게 줬는데도 쿼리가 안 되는** 상황이죠. Lake Formation grant가 빠졌기 때문인데, 이 이야기는 시리즈 마지막 편에서 실제 디버깅 과정으로 풀어갑니다.  
{: .prompt-warning}
...
엔진이 둘이라 "뭘 써야 하나" 고민이 생깁니다. 둘은 경쟁 관계라기보다 **쓰임새가 다릅니다**.

| 기준 | Amazon Athena | Amazon Redshift |
| :--- | :--- | :--- |
| 모델 | 서버리스, 쿼리당 과금 | 클러스터(또는 Serverless), 용량 기반 |
| 적합한 워크로드 | ad-hoc 분석, 로그 탐색, 간헐적 쿼리 | 지속적 BI, 복잡한 대규모 조인/집계, 높은 동시성 |
| 데이터 위치 | 주로 S3 직접 | 자체 스토리지 + Spectrum으로 S3 |
| 운영 부담 | 없음 (서버리스) | (provisioned 시) 클러스터 관리 |

### 4.1. 비용 모델이 다르다

가장 실질적인 차이는 **과금 방식**입니다. Athena는 **쿼리가 스캔한 데이터량(TB) 기준**으로 과금합니다. 그래서 파티션 필터로 스캔 범위를 좁히는 게 곧 비용 절감입니다. 반대로 "가끔 쓰는데 클러스터를 24시간 띄워둘" 필요가 없으니, 간헐적 워크로드에 경제적입니다.

Redshift는 **프로비저닝한 용량(노드) 또는 Serverless RPU 기준**입니다. 데이터를 자체 컬럼 스토리지에 적재해 두고 MPP(massively parallel processing)로 대규모 조인/집계를 빠르게 처리합니다. 매일 무거운 정형 리포트를 돌리거나 동시성이 높은 BI라면 여기가 유리합니다.

### 4.2. Redshift Spectrum과 공존

Redshift도 S3 데이터를 직접 쿼리할 수 있는데, 그 기능이 **Redshift Spectrum**입니다. Spectrum은 Athena처럼 Glue Data Catalog의 외부 테이블(S3/S3 Tables)을 쿼리하며, **같은 Lake Formation 권한 체계**를 따릅니다.

그래서 둘은 배타적이지 않습니다. 같은 Glue Data Catalog와 Lake Formation 위에서, **Athena로 탐색하고 Redshift로 정형 워크로드를 돌리는** 구성이 흔합니다. 간단한 선택 기준은 이렇습니다.
...
이번 편에서 큰 그림을 잡았으니, 다음 편들에서 각 구성요소를 깊이 들어갑니다.

1. **(이번 글) AWS 데이터 분석 스택 Overview** - 구성요소와 전체 흐름
2. **Amazon Athena & Glue Data Catalog** - 서버리스 쿼리 엔진과 메타스토어, fine-grained IAM
3. **S3 Tables & Catalog Federation** - 관리형 Iceberg 레이크하우스, default vs federated 카탈로그
4. **AWS Lake Formation** - IAM 위에 얹히는 데이터 접근 권한, grant와 credential vending
5. **Lake Formation 권한 Deep Dive** - "권한을 다 줬는데 왜 `CATALOG_NOT_FOUND`일까" 실전 디버깅
