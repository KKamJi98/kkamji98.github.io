...
데이터 웨어하우스, 데이터 레이크, 레이크하우스는 나란히 놓고 고르는 세 가지 선택지처럼 소개되곤 합니다. 실제로는 각각 직전 방식이 부딪힌 한계를 풀면서 **순서대로 등장**했고, 그 순서를 모르면 왜 레이크하우스가 파일 위에 굳이 테이블 계층을 다시 얹는지 설명되지 않습니다. 변천사를 시간 축으로 따라가며 각 단계가 어떤 운영 문제를 풀려 했는지 정리합니다. 세 패러다임의 정적인 비교는 [데이터 웨어하우스 vs 데이터 레이크 vs 레이크하우스](/posts/data-warehouse-lake-lakehouse/)에서 따로 다룹니다.
