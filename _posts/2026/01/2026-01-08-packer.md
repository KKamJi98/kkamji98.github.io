...
{% include diagrams/packer-golden-image.html %}
{% include diagrams/download.html png="/assets/img/diagrams/packer-golden-image--81dd84a1881b6dfe.png" %}
...
> **TL;DR**  
> - Packer는 하나의 템플릿으로 AWS AMI, Azure Image, GCP Image, Docker Image 등 여러 플랫폼의 머신 이미지를 같은 구성으로 만들어냅니다.  
> - 구성 요소는 세 가지입니다. 플랫폼별 이미지를 만드는 Builder(`amazon-ebs` 등), 소프트웨어를 설치하는 Provisioner(`shell`, `ansible` 등), 빌드 후 압축이나 태깅을 맡는 Post-processor입니다.  
> - 빌드는 임시 인스턴스를 띄워 SSH로 붙고 Provisioner를 실행한 뒤 이미지를 굽고, 인스턴스와 키페어, 보안그룹 같은 임시 리소스를 정리하는 순서로 진행됩니다.  
{: .prompt-info}
...
5. **Post-processor 실행** (선택)
   - 빌더가 생성한 결과물을 대상으로 후처리
   - 빌드 결과 목록 기록이나 지원되는 형식의 변환 등
...
4. **이미지 생성 및 정리**: AWS `amazon-ebs` 빌더는 구성된 EBS 볼륨의 스냅샷을 참조하는 AMI를 생성합니다. 이후 빌드에 사용한 임시 인스턴스와 키페어, 보안 그룹 등을 정리합니다.

5. **Post-processor 실행**: 빌더가 생성한 결과물을 대상으로 선택적인 후처리를 수행합니다. 예를 들어 `manifest`는 빌드 결과 목록을 기록하고, `docker-push`는 Docker 이미지를 레지스트리에 푸시합니다.
...
- [Packer Docs - Amazon EBS Builder](https://developer.hashicorp.com/packer/integrations/hashicorp/amazon/latest/components/builder/ebs)
- [Packer Docs - Post-processors](https://developer.hashicorp.com/packer/docs/post-processors)
- [HashiCorp - Flight Icons](https://github.com/hashicorp/design-system)
- [AWS Labs - AWS Icons for PlantUML](https://github.com/awslabs/aws-icons-for-plantuml)
