...
![DNS 응답과 애니캐스트 IP와 엣지 캐시 세 계층이 각각 어떤 결정을 내리는지 비교한 그림](/assets/img/sap-c02/dns-global-traffic-decision-layers.webp)
...
![Route 53 health checker가 VPC 밖에 있어 사설 IP에 도달하지 못하는 경계와 CloudWatch alarm을 경유하는 대체 경로](/assets/img/sap-c02/route53-health-check-private-endpoint.webp)
...
![온프레미스와 VPC 사이 DNS 질의 방향에 따라 inbound endpoint와 outbound endpoint가 나뉘는 구조](/assets/img/sap-c02/route53-hybrid-dns-endpoints.webp)
...
![Global Accelerator와 CloudFront가 각각 요청을 처리하는 경로와 캐시 유무의 차이](/assets/img/sap-c02/global-accelerator-cloudfront-paths.webp)
...
![CloudFront origin group에서 primary 요청이 failover 조건을 만나 secondary로 넘어가는 경로와 그 경로를 타지 못하는 요청](/assets/img/sap-c02/cloudfront-origin-failover-path.webp)
