...
![액세스 패턴 여덟 종류와 각각에 대응하는 데이터 계층 서비스, 그리고 그 선택을 강제하는 제약](/assets/img/sap-c02/data-store-access-pattern-matrix.webp)
...
![EBS Multi-Attach 가 하나의 AZ 안에 머무는 구성과, 여러 AZ 의 인스턴스가 함께 마운트하는 Regional EFS 구성](/assets/img/sap-c02/shared-storage-az-boundary.webp)
...
![S3 스토리지 클래스 사이의 단방향 lifecycle 전환 경로와, 상위 클래스로 되돌리는 restore 및 copy 경로](/assets/img/sap-c02/s3-lifecycle-waterfall.webp)
...
![S3 원본 버킷에서 대상 버킷으로 가는 live replication 경로와 Batch Replication 경로, 그리고 어느 쪽으로도 복제되지 않는 항목](/assets/img/sap-c02/s3-replication-scope.webp)
...
![Multi-AZ DB instance 와 Multi-AZ DB cluster 와 read replica 세 가지 배포 형태의 노드 구성과 복제 방식](/assets/img/sap-c02/rds-deployment-topologies.webp)
...
![Aurora Global Database 의 primary 와 secondary 클러스터, 그리고 switchover 와 managed failover 두 승격 경로](/assets/img/sap-c02/aurora-global-switchover-failover.webp)
