...
[이전 글](https://kkamji.net/posts/ethereum-account-nonce-gas/)에서 nonce와 21000 gas 이체를 관측했습니다. 이번에는 Foundry 1.7.1로 Counter를 테스트하고 Anvil에 deploy합니다. 실습은 로컬 체인만 사용합니다.

---

## 1. 테스트가 통과한 뒤에 deploy한다
...
퍼즈 테스트는 `setNumber`에 난수 256개를 넣어 저장값이 그대로인지 확인합니다. deploy 전에 스토리지 규칙을 깨지 못했다는 증적입니다. deploy 후에는 코드를 고쳐서 되돌리기 어렵습니다.

---

## 2. deploy address에는 코드가 있다

`forge create`로 deploy한 address는 `0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512`였습니다. `cast code`는 빈 값이 아니라 964자의 hex였습니다.
...
같은 node에서 EOA를 조회하면 여전히 `0x`입니다. address 형식이 같아서 헷갈리지만, 코드 유무가 계정 종류를 가릅니다.

---

## 3. 호출은 스토리지를 바꾼다

deploy 직후 `number()`는 0이었습니다. `increment()` receipt는 `status=0x1`, `gasUsed=43482`였고, 다시 읽으면 1이었습니다. 단순 이체 21000보다 큰 이유는 스토리지 쓰기가 들어가기 때문입니다.
...
Foundry는 테스트, deploy, 호출을 같은 도구 세트로 묶습니다. `forge test`로 규칙을 확인하고, `forge create`로 코드를 address에 올리며, `cast code`와 `cast call`로 결과를 읽습니다. 이 관측은 Anvil(chainId 31337)에서만 나왔고 실제 자산은 움직이지 않았습니다.
