...
한글 AI 글을 손보는 방법은 크게 두 가지였습니다.

영어권 humanizer(QuillBot, Undetectable AI 등)는 한국어에서 잘 듣지 않습니다. 한글 AI 글의 티는 대부분 영어 번역투에서 나오는데, 영어 문장을 다시 쓰는 도구는 그 조사와 어미 흔적을 대상으로 삼지 않습니다.

모델에 직접 "AI 티 없애줘"라고 지시하는 방법은 네 군데서 흔들립니다.

- 기준이 없다. 무엇을 AI 티로 볼지 매번 모델이 새로 정하므로 같은 글을 두 번 돌리면 결과가 달라진다
- 의미가 드리프트한다. 문장을 다듬다가 주장을 바꾸거나 없던 단정을 넣는다
- 검증이 없다. 얼마나 고쳤는지 확인할 방법이 모델의 자기 보고밖에 없다
- 비용이 일정하다. 이미 잘 쓴 글에도 같은 무게로 처리한다

뒤의 두 항목이 특히 성가십니다. 발행 직전 글에 윤문을 돌리면 결과를 처음부터 다시 읽어야 하고, 그 부담 때문에 결국 안 돌리게 됩니다.
...
이 글을 쓰며 저장소를 직접 확인한 시점은 2026-08-23입니다. 그날 기준으로 최신 릴리스 태그는 `v2.3.2`이고, 설치 경로의 버전 디렉터리가 이 태그와 일치하는지 `claude plugin list`로 먼저 확인하는 편이 안전합니다. 업스트림은 활발하게 움직이는 저장소라 8월 중순에도 플러그인 스킬 위치 정정(`v2.3.2`), 영문 문서 세트 신설, C-8 역치 조정 같은 변경이 이어졌습니다. 버전이 바뀌면 이 글의 경로와 파일 구성도 함께 확인해야 합니다.
...
번역투는 영어 문장 구조를 한글 조사와 어미로 옮긴 흔적입니다. 실제 문장에서는 이렇게 나타납니다.
...
{% include diagrams/download.html png="/assets/img/diagrams/static/ai/humanize/humanize-route-hint--1c9592ce01d76f4d.png" %}
...
마지막 항목의 판정을 모델의 자기 보고가 아니라 파이썬 스크립트가 합니다. `verify_gates.py`가 문자 기준 변경률, S1 목표 달성, 대구 전멸 여부, 수치와 고정 문구 보존 네 축을 검사하고 종료 코드로 결과를 냅니다. 2026-08-23 기준 저장소 HEAD에서는 여기에 서법 보존 축이 하나 더 추가돼 다섯 축으로 움직입니다. 당위("~해야 한다")를 사실 단정("~한다")으로, 추측("~일 수 있다")을 단정으로 바꾸는 것도 게이트가 잡습니다. 윤문은 어휘만 고치는 게 아니라 서법까지 흔들 수 있다는 게 그 추가 이유입니다.

축을 네 개 둔 이유도 실측에서 나왔습니다. 문자 변경률이 2.77%인데 문장 터치율은 29.7%, 대구는 75% 감소한 사례가 있었습니다. 문자 diff만 보면 구조 편집을 놓칩니다.

---

## 7. 무엇이 나아지는가

앞에서 짚은 네 가지에 대응하는 답이 스킬 안에 파일로 들어 있습니다.

| 문제 | 스킬의 대응 |
|---|---|
| 기준이 없다 | 10대 카테고리 70패턴과 심각도를 SSOT 파일로 고정 |
| 의미가 드리프트한다 | 탐지된 구간만 수정, 수치와 고유명사는 대상 제외 |
| 검증이 없다 | `verify_gates.py`가 네 축을 검사해 종료 코드로 판정 |
| 비용이 일정하다 | `route_hint`가 글 상태에 따라 콜 수를 1에서 3으로 조절 |

실무에서 가장 체감되는 쪽은 검증입니다. 변경률이 코드로 나오니 결과를 처음부터 다시 읽지 않고 게이트가 잡은 구간만 확인하면 됩니다. 부분 재실행도 됩니다. "이 문단만", "이 카테고리만 다시" 같은 요청이 같은 run 안에서 처리됩니다.

설계에서 눈에 띄는 부분은 판정 주체입니다. 탐지와 윤문은 모델이 하지만 "너무 고쳤는가"는 모델에게 묻지 않습니다. 파이썬이 원문과 결과를 비교합니다. 흔들려도 되는 판단은 모델에 맡기고 흔들리면 안 되는 판단은 코드에 맡긴 구조입니다.

대가도 있습니다. 코드가 세는 것은 어휘와 카운트뿐이라 리듬 균일성처럼 셀 수 없는 패턴은 결국 모델 진단에 남습니다.
