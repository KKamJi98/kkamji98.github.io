...
comments: true
image:
  path: /assets/img/aws/analytics-stack-07-architecture-history.webp
---

앞선 [데이터 웨어하우스 vs 데이터 레이크 vs 레이크하우스](/posts/data-warehouse-lake-lakehouse/) 글에서 세 가지 저장 패러다임을 정적으로 비교했습니다. 그런데 이 셋은 어느 날 동시에 등장한 것이 아니라, 각각 직전 방식의 한계를 풀면서 **순서대로 진화**한 결과입니다. 이번 글에서는 그 변천사를 시간 축으로 따라가며, 왜 이런 흐름이 생겼는지를 정리합니다.
...
![데이터 분석 아키텍처 변천사 타임라인](/assets/img/aws/analytics-stack-07-architecture-history.webp)
...
AWS 스택에서 이 흐름이 어떤 서비스로 구현되는지는 [이 시리즈의 다른 글들](/posts/aws-analytics-stack-overview/)에서 다룹니다.
