...
Lake Formation이 까다로운 이유는, IAM 정책에서 모든 권한을 Allow로 열어 줘도 쿼리가 `AccessDenied`로 막힐 수 있기 때문입니다. IAM 위에 데이터 접근을 따로 통제하는 게이트가 하나 더 있고, 그 게이트는 IAM과 전혀 다른 방식(grant)으로 동작합니다.
...
```bash
# IAM (정책 기반): identity-based policy의 한 statement
#   "Effect": "Allow",
#   "Action": "glue:GetTable",
#   "Resource": "arn:aws:glue:...:table/sales_db/orders"

# Lake Formation (grant 기반): grant-permissions API/CLI
...
LF grant는 SQL의 `GRANT ... ON ... TO ...`와 개념은 비슷하지만, 실제 인터페이스는 SQL 문이 아니라 `grant-permissions` API/CLI(또는 콘솔)입니다. `--permissions`에는 위 표의 토큰(`SELECT`/`DESCRIBE`/`ALL` 등)을, `--resource`에는 대상 리소스를 JSON으로 지정합니다.

이 둘은 서로를 대체하지 않습니다. LF 등록 데이터는 두 표현이 **모두** 충족돼야 접근이 허용됩니다.
...
IAM은 "이 role이 `glue:GetTable` API를 호출해도 되는가"를 판단합니다. Lake Formation은 그다음 단계에서 "이 role이 `sales_db.orders`라는 데이터에 실제로 접근해도 되는가"를 판단합니다.
...
- LF grant는 있는데 IAM에서 `glue:GetTable`/`glue:GetDatabase` 같은 메타데이터 action이나 `lakeformation:GetDataAccess`가 빠지면 -> **AccessDenied**

특히 첫 번째 경우가 혼동을 일으킵니다. IAM 정책에서 모든 Glue/Athena 권한을 열어 줬는데도 다음 에러가 나오면 LF 게이트에서 막힌 상태입니다.

```text
AccessDeniedException: Insufficient Lake Formation permission(s): Required Describe
```

단서는 메시지 안의 `Lake Formation permission(s)`입니다. IAM이 아니라 LF가 별도 게이트로 접근을 거부했습니다.
...
| `ALL` | 전체 권한 (콘솔에서는 `Super` 라벨로 표기) |

표준 permission 토큰은 `ALL`입니다. Lake Formation 문서와 콘솔이 이 전체 권한을 `Super`라고 부르는 경우가 있어 혼동하기 쉽지만, `grant-permissions` API/CLI에 넘기는 토큰은 `ALL`입니다. 공식 reference의 리소스별 권한 목록도 동일하게 표기합니다.
...
운영 현장에서는 보통 인프라/보안 담당자가 **IAM 측 권한**(API, `GetDataAccess`)을 보강하고, 데이터 자산을 관리하는 **data lake admin이 LF grant**를 부여하는 식으로 책임이 나뉩니다. 한쪽만으로는 쿼리가 동작하지 않으므로 권한을 요청할 때 두 축을 함께 확인합니다.

---

## 4. credential vending과 GetDataAccess

LF 게이트를 통과한 뒤 실데이터를 읽는 단계에서는 Lake Formation의 **credential vending**이 동작합니다.
...
이 구조에서는 분석 사용자에게 데이터 버킷의 S3 권한을 일일이 부여하지 않고 LF grant만으로 접근을 통제합니다. 다이어그램 4~5번 단계에서 LF가 registered role을 assume(4)하고, 그 자격증명으로 S3 Tables 데이터를 읽어(5) 결과를 엔진에 돌려줍니다.

### 4.3. lakeformation:GetDataAccess

credential vending이 동작하려면, 호출자 측에 **`lakeformation:GetDataAccess` IAM 권한**이 있어야 합니다. 이 권한이 있어야 LF가 임시 자격증명 발급 요청을 승인합니다. LF grant(SELECT/DESCRIBE)와 별개인 IAM 측 조건입니다.
...
`glue:Get*` 같은 와일드카드 대신 필요한 action을 단수/복수로 명시하면 최소 권한 관점에서 안전합니다. 단일 객체 조회(`GetTable`/`GetDatabase`/`GetCatalog`)와 목록 조회(`GetTables`/`GetDatabases`/`GetCatalogs`)가 별개의 action이라, 둘 중 하나만 넣으면 카탈로그/데이터베이스 탐색이나 테이블 단건 조회 중 한쪽이 막힐 수 있습니다. 공식 reference의 `DESCRIBE` permission도 grantee가 추가로 `glue:GetTable`(테이블),`glue:GetDatabase`(데이터베이스)를 가져야 한다고 명시합니다.
...
S3 Tables를 Glue Data Catalog에 통합할 때 **권한 모드를 선택**하고, 이 선택에 따라 "IAM + LF" 게이트 구성이 달라집니다(나중에 전환 가능).
...
레이크하우스 데이터가 Lake Formation access control mode로 구성돼 있다면, IAM만 아무리 열어도 LF grant 없이는 접근할 수 없습니다.

### 5.1. IAMAllowedPrincipals와 hybrid access mode

여기서 한 가지 함정이 있습니다. Lake Formation은 backward compatibility를 위해, 기본적으로 모든 데이터베이스/테이블에 `IAMAllowedPrincipals`라는 그룹에 `Super`(전체) 권한을 부여합니다. 이 그룹 권한이 리소스에 남아 있으면, LF grant를 따로 안 해도 IAM 정책만으로 접근이 열려 버립니다. 즉 "LF가 켜져 있는데 왜 IAM만으로 되지?"의 원인이 보통 이것입니다.
...
LF grant를 실제 게이트로 작동시키려면 다음 둘 중 하나가 필요합니다.

- **`IAMAllowedPrincipals`의 `Super` 권한을 revoke**: 해당 리소스를 완전히 LF 권한으로만 통제. 단, IAM만으로 접근하던 기존 워크로드가 끊길 수 있습니다.
- **hybrid access mode**: S3 location 등록 시 선택하는 모드로, opt-in한 principal만 LF 권한을 적용받고 나머지는 기존 IAM 권한으로 계속 접근합니다. 기존 사용자를 끊지 않고 점진적으로 전환할 때 사용합니다.
...
레이크하우스(S3 Tables federated catalog)가 처음부터 LF 권한으로만 통제되도록 설정돼 있다면, `IAMAllowedPrincipals` 권한이 없으므로 LF grant가 없는 principal은 그대로 막힙니다.

### 5.2. end-to-end - AccessDenied에서 성공까지

분석 role이 LF 등록 테이블 `sales_db.orders`를 Athena로 조회하려 합니다.

1. **막힘**: IAM 정책에 `athena:*`, `glue:Get*`, `lakeformation:GetDataAccess`까지 다 열려 있는데도 쿼리가 실패합니다.

```text
AccessDeniedException: Insufficient Lake Formation permission(s): Required Describe
```

메시지에 `Lake Formation permission(s)`가 있으므로 LF 게이트에서 막혔습니다(2.2 참고).

2. **현재 grant 확인**: data lake admin이 해당 role에 무슨 LF 권한이 있는지 조회합니다.
...
반환이 비어 있으면 LF grant가 없는 것이 확정됩니다.

3. **grant 부여**: admin이 읽기 전용 권한을 부여합니다.
...
4. **성공**: 같은 Athena 쿼리가 통과합니다. IAM 두 축(메타데이터 action + `GetDataAccess`)과 LF grant(`SELECT`+`DESCRIBE`)가 모두 충족됐기 때문입니다. 실데이터 읽기는 4.2의 credential vending이 등록 role로 처리하므로, 이 role에 데이터 버킷의 S3 권한을 따로 줄 필요는 없습니다.
...
- AWS Lake Formation Developer Guide - Lake Formation permissions reference (리소스별 권한 토큰 ALL (Super)/SELECT/DESCRIBE, IAMAllowedPrincipals, grant CLI 예시): <https://docs.aws.amazon.com/lake-formation/latest/dg/lf-permissions-reference.html>
- AWS Lake Formation Developer Guide - Hybrid access mode (IAM/LF 이중 경로, opt-in): <https://docs.aws.amazon.com/lake-formation/latest/dg/hybrid-access-mode.html>
- AWS CLI Command Reference - lakeformation list-permissions (principal/resource 별 부여 권한 조회): <https://docs.aws.amazon.com/cli/latest/reference/lakeformation/list-permissions.html>
