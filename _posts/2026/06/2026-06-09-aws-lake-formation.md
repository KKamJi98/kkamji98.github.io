...
따라서 LF grant를 실제 게이트로 작동시키려면 다음 둘 중 하나가 필요합니다.
...
지금까지의 조각을 하나의 시나리오로 이어 봅니다. 분석 role이 LF 등록 테이블 `sales_db.orders`를 Athena로 조회하려 합니다.
...
메시지에 `Lake Formation permission(s)`가 있으므로 IAM이 아니라 LF 게이트에서 막힌 것입니다(2.2 참고).
