...
- **원본 문서의 탭 구조 직접 색인**: 현재는 원본의 사본을 색인하고 있어 citation의 상위 경로가 균일화된 형태로 나옵니다. 원본(챕터별 탭 구조)을 직접 색인하면 출처 표기가 탭(챕터) 기준으로 더 명확해집니다. 문서 구조가 곧 검색 품질의 상한이라는 점을 이번에 실감했습니다.
- **채널 확산**: 현재는 봇을 초대한 채널에서만 동작합니다. 다른 채널로 넓히려면 봇 초대를 opt-in으로 받는 방식으로 확장합니다.
...
다음 단계는 평가 질문셋(골든 Q&A) 기반 튜닝과 문서 탭 구조 직접 색인입니다. 결과가 쌓이면 후속 글로 정리하겠습니다.
