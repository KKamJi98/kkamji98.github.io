...
처음에는 Ghostty 창과 tab을 여러 개 열고 Claude Code, Codex, Hermes를 각각 실행했습니다. 작업이 두세 개일 때는 충분했습니다. Ghostty는 macOS native user interface(UI), tab, split을 제공하므로 일반 terminal로 사용하기에 적합합니다.
...
| Terminal rendering | shell, terminal user interface(TUI), text rendering | cmux의 Ghostty 기반 terminal, Ghostty fallback |
...
| agent native resume | 지원 agent의 session identifier(ID)와 대화 맥락 | 실행 중 child process의 memory와 open file descriptor |
...
{% include diagrams/download.html png="/assets/img/diagrams/static/ai/agentic-development-workspace--b5f946bb410b7750.png" %}
