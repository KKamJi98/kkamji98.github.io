...
유료 리소스를 쓰려면 Project를 **Cloud Billing 계정**에 연결해야 합니다. 결제 계정과 Project는 1:N 관계라서, 하나의 결제 계정이 여러 Project의 비용을 함께 청구받도록 묶을 수 있습니다. 공식 문서의 설명입니다.
...
입문 단계에서는 "상위 권한은 allow로 하위에 합집합 상속되고, deny 정책은 그보다 우선해 차단한다" 정도만 기억하면 됩니다.
...
Project ID는 생성 후 변경이 불가능하고, **삭제한 프로젝트의 ID까지 포함해 한 번 사용된 ID는 영구적으로 재사용할 수 없습니다**. 공식 문서가 ID 요건에서 명시하는 부분입니다.
...
- Google Cloud Docs - Understanding hierarchy evaluation (조직 정책 상속/병합): <https://cloud.google.com/resource-manager/docs/organization-policy/understanding-hierarchy>
- Google Cloud Docs - Enable, disable, or change billing for a project (결제 계정 연결): <https://cloud.google.com/billing/docs/how-to/modify-project>
