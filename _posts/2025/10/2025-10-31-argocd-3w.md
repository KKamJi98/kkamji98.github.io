...
**Argo CD를 사용한 CD** (Continuous Delivery) 과정에 대해 다루겠습니다.

실습 환경은 [Jenkins + ArgoCD 실습 환경 구축]({% post_url 2025/10/2025-10-26-jenkins-ci-cd-env-3w %})의 실습 환경을 사용합니다.
...
{% include diagrams/static/ci-cd/ci-cd-study/argocd-architecture-overview.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/argocd-architecture-overview--3f3e72d2791d7996.png" %}
...
{% include diagrams/static/ci-cd/ci-cd-study/argo-cd-component-architecture.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/argo-cd-component-architecture--b347b8c759a22ffd.png" %}
...
즉, "**Git에 정의된 소스 -> 실제 적용 가능한 Kubernetes Manifest**"로 변환하는 역할을 합니다.
...
- **Redis**  
  - Kubernetes API 및 Git 요청을 줄이기 위한 캐시 레이어  
  - 빈번한 조회에 대한 성능 개선 역할
...
![Argo CD Change Password](/assets/img/ci-cd/ci-cd-study/argocd-change-password.webp)
...
- PRUNE : Git에서 자원 삭제 후 배포시 Kubernetes에서는 삭제되지 않으나, 해당 옵션을 선택하면 삭제시킴
...
SYNC 클릭 -> 생성될 리소스 확인 -> Kubernetes(Live) 반영 확인
...
### 6.2. (참고) Kubernetes Finalizers 와 Argo Finalizers 동작
...
![Kubernetes Blog - Using Finalizers to Control Deletion](/assets/img/ci-cd/ci-cd-study/kubernetes-finalizer-blog.webp)
...
- **ArgoCD Finalizers의 목적**
  ...
  2. **의도치 않은 삭제 방지**: finalizer가 없으면 실수로 **Argo App을 삭제**해도 **Kubernetes 리소스가 남아 혼란**이 생길 수 있습니다. finalizer는 이를 **방지**합니다.
