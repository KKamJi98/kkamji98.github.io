...
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/argo-multi-cluster-architecture--42932e370408ecd3.png" %}
...
**Argo CD**를 사용하여 멀티 클러스터 환경을 구축하고 관리하는 방법을 실습해 보았습니다. `kind`를 이용해 로컬에서 관리용(Mgmt), 운영용(Prod), 개발용(Dev) 클러스터를 구성하고, **Argo CD**에 외부 클러스터를 등록하여 중앙에서 애플리케이션을 배포하고 관리하는 과정을 경험했습니다. 이를 통해 **Argo CD**가 단일 클러스터뿐만 아니라 멀티 클러스터 환경에서도 강력한 GitOps 도구로 활용될 수 있음을 확인했습니다.
