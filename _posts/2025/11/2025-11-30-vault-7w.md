...
서비스가 많아질수록 DB Password, API Token, TLS 인증서, Cloud Access Key 같은 시크릿은 Git Repository, CI/CD 변수, Kubernetes Secret, 개발자 로컬 환경 등 여러 곳으로 흩어지기 쉽습니다. 이렇게 분산된 시크릿은 누가 접근했는지 추적하기 어렵고, 유출 이후 회전하거나 폐기하는 작업도 느려집니다. HashiCorp Vault가 해결하는 문제와 핵심 구성요소의 연결, 운영 환경 설계 기준을 정리합니다.

> **TL;DR**  
> Vault는 인증(Authentication), 인가(Authorization), 동적 발급(Dynamic Secrets), 암호화(Transit), 감사(Audit)를 하나의 흐름으로 묶어 시크릿 생명주기를 관리하는 플랫폼입니다. 저장은 그 흐름의 한 부분입니다.  
{: .prompt-info}
...
{% include diagrams/static/ci-cd/ci-cd-study/vault-cia-triad.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/vault-cia-triad--60476fdd9145c2a8.png" %}
...
- Vault 적용 포인트:
  ...
  - 감사 로그에 모든 API 호출을 남겨 변경 이벤트를 사후에 추적합니다.
...
{% include diagrams/static/ci-cd/ci-cd-study/vault-aaa.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/vault-aaa--f140b2c2064ccb53.png" %}
...
- Vault 적용 포인트:
  ...
  - 동적 자격증명(DB, 클라우드) 사용 시 `lease`와 `revoke`로 수명 종료를 강제합니다.
...
- 핵심: 요청과 응답을 감사 로그로 남겨 사후에 추적합니다. 감사 로그 자체의 보존과 무결성은 별도 로그 플랫폼과 접근 제어로 설계해야 합니다.
...
{% include diagrams/static/ci-cd/ci-cd-study/vault-access-management-overview.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/vault-access-management-overview--42471d6d9856924d.png" %}
...
- **3. 얼마 동안 (How Long) : 접근 시간 제어 (TTL)**
  - 핵심 개념: TTL(Time To Live, 유효 기간)로 권한을 제한합니다.
...
{% include diagrams/static/ci-cd/ci-cd-study/vault-overview.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/vault-overview--bc9c4659ab29c972.png" %}
...
{% include diagrams/static/ci-cd/ci-cd-study/vault-core-workflow.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/ci-cd/ci-cd-study/vault-core-workflow--4aa921e8b2afe04a.png" %}
...
Vault를 이해할 때는 `Auth Method`, `Token`, `Policy`, `Secret Engine`, `Lease`, `Audit Device`, `Storage`, `Seal`을 분리해서 봅니다. 각각의 역할이 명확해야 운영 중 권한 문제나 장애 원인을 빠르게 좁힐 수 있습니다.

`Auth Method`와 `Secret Engine`의 경계를 먼저 잡습니다. Auth Method는 사람이나 워크로드의 신원을 검증해 client token을 발급하는 입구이고, Secret Engine은 인증과 인가를 통과한 요청을 처리해 정적 값, 동적 자격증명, 인증서, 암호화 기능을 제공하는 backend입니다. token은 그 자체가 시크릿이므로 값은 로그나 ticket에 남기지 않고, policy와 lease를 함께 확인해야 실제 권한과 수명을 판단할 수 있습니다.
...
Vault는 요청자가 누구인지 먼저 확인합니다. 사람 사용자는 OIDC, LDAP, GitHub로 인증하고, 애플리케이션은 Kubernetes Service Account, AWS IAM, AppRole로 인증합니다. 인증이 성공하면 Vault는 정책이 연결된 `token`을 발급하고, 이후 요청은 이 token의 권한 범위 안에서만 처리됩니다.

운영 환경에서는 인증 방식을 사용자와 워크로드 기준으로 분리해야 합니다. 예를 들어 운영자가 CLI로 접근하는 흐름과 Pod가 시크릿을 읽는 흐름을 같은 token으로 처리하면 감사 추적과 권한 회수가 어려워집니다. 초기 `root token`은 모든 권한을 가지므로 설정을 마친 뒤에는 보관 절차를 분리하고 일반 운영에는 사용하지 않습니다.

### 2.2. Policy와 Path 기반 권한

Vault의 권한은 path 중심으로 동작합니다. Secret Engine을 `secret/`, `database/`, `pki/`, `transit/` 같은 path에 mount하고, policy에서 해당 path에 허용할 `read`, `create`, `update`, `delete`, `list` 등의 capability를 정의합니다.
...
위 예시는 애플리케이션이 `secret/data/app/prod/*` 아래의 KV secret을 읽고, `database/creds/app-readonly`에서 동적 DB 계정을 발급받을 수 있도록 허용합니다. 반대로 policy에 없는 path는 기본적으로 거부됩니다. 이 특성 때문에 Vault 권한 설계는 허용할 대상만 명시하는 방식이 됩니다.
...
정적 시크릿은 운영자가 값을 저장하고 애플리케이션이 읽는 방식입니다. 반면 동적 시크릿은 요청 시점에 Vault가 외부 시스템에 실제 계정이나 자격증명을 생성하고, lease가 만료되거나 revoke되면 해당 자격증명을 폐기합니다. 장기 자격증명을 줄이는 것이 목적이라면 동적 시크릿을 먼저 검토합니다.

### 2.4. Lease, Renew, Revoke

Vault는 시크릿을 발급한 뒤 수명주기까지 추적합니다. 동적 시크릿은 lease를 가지며, lease에는 TTL과 갱신 가능 여부가 포함됩니다. 애플리케이션은 TTL 안에 시크릿을 사용하고, 장기 실행 작업이라면 renew를 수행하거나 새 시크릿을 다시 발급받아야 합니다.
...
이 흐름은 유출된 자격증명이 얼마나 오래 유효한지에 바로 답합니다. 정적 password가 몇 달 동안 유효한 구조보다, 30분짜리 동적 계정이 자동 폐기되는 구조가 사고 반경을 훨씬 작게 만듭니다.
...
운영 환경에서는 수동 unseal 절차가 장애 복구 시간을 늘릴 수 있어 AWS KMS, GCP Cloud KMS, Azure Key Vault, Hardware Security Module(HSM) 같은 외부 KMS와 연계한 `Auto Unseal`을 검토합니다. 다만 Auto Unseal을 사용하면 KMS 접근 권한, KMS 장애, recovery key 보관 절차까지 함께 설계해야 합니다.
...
가장 먼저 적용하기 쉬운 방식은 Git, CI 변수, Kubernetes Secret 등에 흩어진 정적 시크릿을 Vault KV로 모으는 것입니다. 이 단계에서는 시크릿 저장 위치를 줄이고 누가 어떤 값을 읽었는지 감사 로그로 추적합니다.
...
정적 시크릿 중앙화만으로도 하드코딩과 복사본 문제를 줄일 수 있지만, password 자체가 오래 살아있다는 문제는 남습니다. DB, Cloud, PKI처럼 Vault가 동적으로 발급할 수 있는 대상은 다음 단계에서 동적 시크릿으로 전환합니다.
...
이 방식은 유출 대응이 명확합니다. 특정 lease를 revoke하면 해당 DB 계정만 폐기할 수 있고, 감사 로그에서 어떤 entity가 언제 발급받았는지도 추적할 수 있습니다.

### 3.3. Transit을 이용한 Encryption as a Service

Transit Secrets Engine을 쓰면 애플리케이션이 암호화 키를 직접 보관하지 않습니다. 애플리케이션은 평문을 Vault에 보내 암호문을 받고, 복호화가 필요할 때도 Vault API를 호출합니다. 키는 Vault 안에 남고 애플리케이션은 키를 직접 다루지 않습니다.
...
이 패턴은 애플리케이션 DB에 민감 데이터를 저장해야 하지만 키 관리 체계를 애플리케이션 코드와 분리하고 싶을 때 유용합니다. 서명/검증 기능을 이용하면 배포 아티팩트나 메시지 무결성도 검증할 수 있습니다.
...
여기서 Kubernetes Secret을 최종 저장소로만 볼지, Vault를 source of truth로 둘지 결정해야 합니다. Vault를 source of truth로 두면 시크릿 회전, audit, revoke 흐름은 Vault 중심으로 관리하고, Kubernetes에는 필요한 범위의 값만 동기화하는 구조가 됩니다.
...
Vault는 시크릿 저장소로 시작해도 되지만, 시크릿의 전체 생명주기를 통제할 때 효과가 큽니다. 인증된 주체에게만 필요한 권한을 짧게 부여하고, 사용 기록을 감사 로그로 남기며, 만료와 폐기를 자동화하면 시크릿 확산과 장기 자격증명 문제를 크게 줄일 수 있습니다.

운영 환경에서는 기능을 많이 켜기보다 신원, 권한, 수명, 감사, 복구를 한 흐름으로 설계해야 합니다. 감사 장치는 가용성에도 영향을 주고, 동적 시크릿은 lease ID로 갱신하고 폐기합니다. 이 두 동작을 운영 절차와 장애 훈련에 반영해야 합니다. 도입 순서는 정적 시크릿 중앙화, 사람과 워크로드 인증 분리, 동적 시크릿 전환, 감사와 복구 훈련이 현실적입니다. 다음 글인 [HashiCorp Vault/VSO in Kubernetes](/posts/valut-vso-in-k8s-8w/)에서는 Kubernetes 위에 Vault를 설치하고, Vault Secrets Operator를 통해 애플리케이션에 시크릿을 전달하는 실습을 다룹니다.
