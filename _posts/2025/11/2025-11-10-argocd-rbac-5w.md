...
**예제로 배우는 Argo CD** 책의 4장 `접근 제어`를 다루겠습니다.

> **TL;DR**  
> - Argo CD의 로컬 사용자, 서비스 계정, RBAC 정책, SSO 연동 흐름을 접근제어 관점에서 정리합니다.  
> - admin 계정 비활성화, 최소 권한 계정 생성, token 기반 자동화, policy.csv 작성 방식이 핵심입니다.  
> - 운영에서는 사람 계정과 자동화 계정을 분리하고 프로젝트와 역할 단위로 권한 범위를 좁힙니다.  
{: .prompt-info}

##### 다룰 내용

1. Argo CD에서 어떻게 사용자의 접근을 제어하는지
2. 터미널이나 CI/CD 파이프라인에서 CLI로 연결하는 옵션에는 어떤 것들이 있는지, 어떻게 RBAC가 적용되는지
...
오픈 웹 애플리케이션 보안 프로젝트(OWASP, Open Web Application Security Project)의 가장 유명한 프로젝트는 웹 애플리케이션 보안에서 중요하게 고려해야 할 위험 목록인 OWASP TOP 10(<https://owasp.org/www-project-top-ten/>)입니다. 해당 목록은 몇 년에 한 번씩 업데이트 되는데 현재 최신 버전인 2021년 버전에는 **취약한 접근제어**(Broken Access Control)가 1위를 차지했습니다. 사용자의 액세스 종류를 적절하게 설정해 최소 권한 원칙을 위반하지 않도록 관리해야 한다는 뜻입니다.

### 1.1. 관리자와 로컬 사용자

클러스터에 Argo CD를 설치하면 admin 사용자만 생성됩니다. Argo CD `2.0.0` 이후 버전에서는 해당 admin 사용자 계정의 비밀번호가 `argocd-initial-admin-secret`이라는 시크릿에 저장됩니다.
...
배포 직후에는 해당 Password를 사용해 Web UI나 CLI에서 admin 계정으로 로그인할 수 있습니다. Admin 계정은 초기 구성 시에만 사용하고 이후에는 로컬 사용자 계정으로 전환하거나 SSO 통합을 구성하는 것을 권장합니다. 초기 비밀번호는 Web UI의 `User Info` 탭에 있는 `UPDATE PASSWORD` 버튼이나 `argocd account update-password` 명령어로 변경합니다.
...
클러스터 배포 직후 해야 할 일은 `admin` 관리자 계정을 비활성화하는 것입니다. 관리자 계정은 권한이 막강합니다. 일에 필요한 최소한의 권한만 줘야 한다는 최소 권한 원칙(<https://en.wikipedia.org/wiki/Principle_of_least_privilege>)을 지키려면 admin 계정을 비활성화해야 합니다. 비활성화 전에 시스템 접근과 일상 작업을 담당할 최소 권한 로컬 사용자를 만들어야 합니다. Argo CD에 그룹이나 팀이 아니라 그냥 접근해야 하는 경우를 위해 항상 모두를 위한 전용 사용자 계정을 만들어 두는 것이 좋습니다. 만약 누군가 팀을 떠나 시스템 접근 권한이 없어지는 경우 해당 계정을 비활성화하거나 삭제해야 하기 때문입니다. 아래와 같이 `argocd-cm`이라는 ConfigMap 리소스를 수정해 `alice`라는 이름으로 사용자를 생성하고 UI와 CLI 접근을 모두 허용하겠습니다.
...
현재 alice 계정은 아무런 권한이 없어 로그인은 되지만 애플리케이션이나 클러스터 목록을 조회해도 볼 수 없고 빈 내용만 표시됩니다. 권한을 부여하는 방법은 두 가지입니다.

1. 사용자에게 특정 권한을 부여
2. 사용자에게 권한이 부여되어 있지 않거나 부여된 권한을 찾을 수 없는 경우 부여할 기본 정책 설정

기본 정책을 읽기 전용으로 설정하고 액세스 토큰을 사용할 때 특정 권한을 부여받으려면 `argocd-rbac-cm` ConfigMap 리소스를 수정해야 합니다.
...
이제 마지막으로 admin 사용자를 비활성화하겠습니다. `argocd-cm` ConfigMap 에서 `admin.enabled` 필드를 `false` 로 변경하면 됩니다.
...
서비스 어카운트는 CI/CD 파이프라인과 같은 자동화 프로세스를 시스템에 인증하는 데 사용하는 계정입니다. 개인 사용자 계정이 비활성화되거나 권한이 변경되면 파이프라인이 실패하므로 서비스 어카운트는 개인 사용자 계정에 종속되어서는 안 됩니다. 서비스 어카운트에는 최소 권한 원칙을 엄격하게 적용해야 하며 파이프라인에서 수행하는 작업 범위를 초과하는 권한을 가져서는 안 됩니다.

Argo CD에서는 서비스 어카운트를 생성하는 방식이 다음 두 가지로 구분됩니다.

1. 로그인 기능은 제거하고 API 키만을 사용하는 로컬 사용자(Local User) 방식
    이 방식은 Argo CD 내에서 별도의 로컬 계정을 생성한 뒤 UI 로그인을 비활성화하고 API 토큰만 발급받아 사용합니다. 주로 CI/CD 파이프라인 또는 자동화 스크립트가 Argo CD API에 접근할 때 사용하며 불필요한 로그인 권한을 제거해 보안을 강화합니다.

2. 프로젝트 역할(Project Role)을 생성하고 해당 역할에 토큰(Token)을 할당하는 방식
    Argo CD의 Project는 각 애플리케이션 그룹별 RBAC(Role-Based Access Control)을 제공하며 특정 Project 내에 역할(Role)을 정의하고 그 역할에 인증 토큰을 발급할 수 있습니다. 이 방식은 멀티테넌시(Multi-tenancy) 환경에서 프로젝트 단위로 권한을 분리하거나 특정 애플리케이션 그룹에만 제한된 권한이 필요한 파이프라인에 적합합니다.

두 방식 모두 사용자 계정과 분리된 독립적인 서비스 계정을 만들 수 있으며, 자동화 파이프라인이 요구하는 권한 범위에 따라 적절한 모델을 선택해야 합니다. 특히 Argo CD는 RBAC가 강력하므로 서비스 어카운트를 설계할 때 애플리케이션 단위 또는 Project 단위로 권한을 세분화해 불필요한 접근 권한이 부여되지 않도록 주의해야 합니다.

### 2.1. 로컬 서비스 어카운트

API Key만 활성화된 별도의 로컬 계정을 만들어 보겠습니다. 이 사용자는 UI나 CLI 암호가 없어 API 키를 생성한 뒤에만 접근합니다. 특정 애플리케이션의 동기화를 시작하는 권한처럼 좁은 범위의 권한만 부여하겠습니다.

먼저 아래와 같이 `argocd-cm` ConfigMap을 수정해 `gitops-ci` 라는 서비스 어카운트 사용자를 추가하겠습니다.
...
`gitops-ci` 계정 생성은 원활하게 끝났지만 액세스 토큰을 만들려 하자 권한 부족으로 permission denied 가 발생했습니다. admin 계정을 다시 활성화하는 방법도 있지만 다시 비활성화하는 것을 잊어버리거나 오랫동안 admin 계정이 활성화 상태로 머물 수 있기 때문에 좋은 방법은 아닙니다. alice 사용자에게 계정 업데이트 권한을 할당하겠습니다. 권한은 `argocd-rbac-cm` ConfigMap에 `user-update` 라는 새 역할을 정의한 뒤 해당 역할을 사용자에게 할당해 부여합니다.

> `argocd-rbac-cm`의 policy.csv에서 p는 정책을 정의할 때 쓰고, g는 사용자 또는 그룹에 역할을 연결할 때 사용합니다.  
{: .prompt-tip}
...
새로 생성된 서비스 어카운트는 별도로 권한을 부여하지 않는 한 기본적으로 읽기 전용 권한만 가집니다. 읽기 전용 토큰으로는 실제 자동화 작업을 수행하기 어렵습니다. 일반적으로 새 클러스터 등록 또는 등록 해제, 사용자 생성 또는 삭제, 애플리케이션 생성 또는 동기화와 같은 작업을 수행해야 하므로 적절한 역할(Role)을 부여해야 합니다.
...
프로젝트 역할(Project Role)은 서비스 어카운트가 사용할 수 있는 두 번째 옵션입니다. Argo CD의 애플리케이션 프로젝트는 역할(Role)로 애플리케이션 정의에 다양한 제약 조건을 적용할 수 있습니다. 예를 들어 애플리케이션이 상태를 가져오는 리포지터리, 배포 대상 클러스터, 허용된 네임스페이스를 지정할 수 있으며 배포가 가능한 리소스 유형을 제한할 수도 있습니다. 특정 프로젝트에서는 애플리케이션이 Secret 리소스를 생성하거나 수정하지 못하도록 막을 수도 있습니다. 이 외에도 프로젝트 단위로 새로운 역할을 만들고 해당 역할에 수행 가능한 작업 범위를 정의한 뒤 토큰을 발급해 서비스 어카운트 또는 자동화 파이프라인에서 사용할 수 있습니다.

Argo CD를 설치하면 `default`라는 기본 프로젝트가 함께 생성되며 이 프로젝트에는 애플리케이션 제한이 전혀 없습니다. (`*` 설정으로 모든 리포지터리, 모든 네임스페이스, 모든 리소스 유형을 허용)
...
이제 프로젝트 기반 토큰 사용 방식을 확인하기 위해 새 프로젝트를 만들고 기존 Argo CD 애플리케이션 일부를 이 프로젝트에 연결해 보겠습니다. 새로운 프로젝트를 생성한 후에는 프로젝트 내에서 사용할 역할을 정의하고 역할에 필요한 권한을 부여한 뒤 토큰을 생성하는 순서로 진행합니다.
...
alice 사용자 계정으로 만든 애플리케이션을 `sync`하려 했지만 권한이 부족해 PermissionDenied 에러가 발생했습니다. `argocd-rbac-cm` ConfigMap을 수정해 필요한 권한을 추가하고 역할에 토큰을 생성한 뒤 해당 토큰으로 Sync를 다시 시도하겠습니다.
...
프로젝트 역할(Project Role) 기반으로 생성되는 모든 토큰은 해당 역할 내에 저장되며 토큰이 마지막으로 사용된 시점과 교체가 필요한 시기를 확인할 수 있습니다. 필요한 경우 토큰에 만료 일자를 지정해 일정 기간 동안만 사용하도록 구성할 수도 있습니다. 다만 만료 주기를 지나치게 짧게 설정하면 관리자가 주기적으로 토큰을 재발급해야 하므로 운영 부담이 늘어납니다.
...
프로젝트 역할 기반으로 생성된 토큰에는 만료 시각(exp)을 설정할 수 있으며 이를 기준으로 교체(로테이션) 시점을 관리할 수 있습니다. 토큰의 실제 사용 여부나 마지막 사용 시점은 `argocd-server`의 접근 로그, Kubernetes Audit 로그, 혹은 Argo CD 앞단에서 동작하는 프록시나 게이트웨이(NGINX Ingress, Envoy, API Gateway 등)의 Access 로그를 통해 확인할 수 있습니다. 이러한 로그에는 요청에 사용된 토큰의 식별 정보(sub 또는 jti)가 기록되므로 특정 토큰이 언제 마지막으로 사용되었는지 추적할 수 있습니다. 이렇게 확보한 사용 기록과 미리 정의한 만료 정책을 함께 놓고 토큰 로테이션 주기를 정하는 방식을 권장합니다.

---

## 3. SSO (Single Sign-On)

SSO를 사용하면 중앙 인증(central authentication)으로 한 번 로그인한 뒤 여러 독립적인 애플리케이션의 권한을 함께 부여받습니다. 예를 들어 `argocd.mycompany.com`에 접근하려 하면 `argocd.mycompany.com`은 외부 공급자를 신뢰해 사용자의 신원을 확인합니다. `argocd.mycompany.com`의 접근 유형은 외부 마스터 시스템에서 사용자의 소속 그룹 또는 계정 설정에 따라 제어할 수 있습니다.

SSO를 도입하면 사용하는 모든 애플리케이션마다 각각의 암호를 두지 않아도 되고 하나의 대시보드에서 모든 것을 제어하며 구성원을 쉽게 추가하고 삭제(onboarding/offboarding)할 수 있어 보안 이점이 있습니다.

Argo CD는 두 가지 방법으로 SSO를 제공합니다.

1. 기본적으로 설치되는 Dex OIDC 공급자 사용
2. Dex 설치 없이 다른 OIDC 공급자로 Argo CD를 직접 사용

SSO를 활성화한 상태에서 로그인할 수 있는 로컬 계정이 없고 관리자가 비활성화된 경우 사용자/비밀번호 입력 양식이 UI에서 사라지고 SSO 로그인 버튼만 남습니다.
...
Keycloak은 애플리케이션에 초점을 맞춘 **오픈 소스 ID 및 접근(권한) 관리 도구**입니다. 사용자는 Keycloak에서 인증을 받고 애플리케이션은 Keycloak이 발급한 토큰으로 권한을 검증합니다. 애플리케이션은 사용자 자격 증명(아이디/비밀번호)을 직접 관리하지 않고도 안전한 인증과 인가를 제공합니다.  
> [Keycloak Docs](https://www.keycloak.org/)  

Keycloak의 주요 특징은 다음과 같습니다.

- **강력한 로그인/인증 기능 제공**
  - Keycloak은 완전히 커스터마이징 가능한 로그인 페이지를 제공하며 암호 복구, 주기적인 암호 변경, 이용 약관 동의, MFA(다단계 인증)도 함께 지원합니다.
  - 이 기능들을 애플리케이션마다 직접 구현할 필요가 없고 Keycloak 쪽 설정만으로 공통 로그인 UX를 구성합니다.
- **애플리케이션이 자격 증명을 직접 다루지 않음**
  - Keycloak에서 인증을 처리하면 각 애플리케이션은 서로 다른 인증 메커니즘을 구현할 필요가 없고 비밀번호 저장과 관리에 딸린 보안 이슈에서도 자유로워집니다.
  - 애플리케이션은 사용자 자격 증명에 직접 접근하지 않고 필요한 정보에만 접근할 수 있는 **보안 토큰(JWT 등)**을 사용합니다.
- **Single Sign-On(SSO) 및 세션 관리**
  - Keycloak이 SSO를 제공하므로 사용자는 한 번만 인증하면 여러 애플리케이션에 접근할 수 있습니다.
...
- **표준 프로토콜 기반**
  ...
  - 덕분에 여러 언어와 프레임워크, 플랫폼이 제공하는 표준 클라이언트 라이브러리로 쉽게 연동할 수 있습니다.
- **유연한 사용자 저장소 및 연동 기능**
  - Keycloak은 자체 사용자 데이터베이스를 기본으로 제공하지만 기존 인증 인프라와도 손쉽게 통합할 수 있습니다.
  - **Identity Brokering** 기능으로 소셜 로그인(구글, 깃허브 등)이나 다른 엔터프라이즈 ID 공급자의 계정을 그대로 연동할 수 있습니다.
  ...
  - 또한 Active Directory 및 LDAP 서버와 같은 기존 사용자 디렉터리와 통합하는 **User Federation** 기능을 제공합니다.
    ...
    > User Federation
- **경량, 고가용성, 확장성**
  - Keycloak은 비교적 가볍고 설치가 쉬우며 클러스터링으로 고가용성을 확보합니다.
  - 여러 데이터 센터에 걸친 클러스터링도 지원하므로 이중화 범위를 더 넓힐 수 있습니다.
  - 커스텀 코드(확장 포인트)로 사용자 정의 인증 메커니즘, 사용자 저장소, 토큰 연동 로직을 확장할 수 있습니다.

#### 3.1.2. OpenID Connect의 권한 부여 코드 흐름

Argo CD를 Keycloak과 직접 연동하면 구성은 다음과 같습니다.
...
![OpenID Connect authorization code flow simplified](/assets/img/ci-cd/ci-cd-study/openid-connect-authorization-code-flow-simplied.webp)
...
앞에서 살펴본 것처럼 Keycloak은 OAuth 2.0 / OpenID Connect 기반의 ID 및 접근 관리 도구이며 Argo CD는 OIDC로 Keycloak과 직접 연동합니다.  
연동을 마치면 Argo CD는 Keycloak을 **외부 인증 서버(IdP)** 로 사용하고 사용자는 Keycloak 계정(또는 연동된 사내 디렉터리/소셜 계정)으로 Argo CD에 로그인합니다.

Argo CD와 Keycloak을 연동하는 전체 절차(클라이언트 생성, 리다이렉트 URI 설정, OIDC 설정, 그룹 기반 RBAC 연동 등)는 아래 공식 문서를 따르길 권장합니다.
...
- [Argo CD Docs - User Management / Keycloak](https://argo-cd.readthedocs.io/en/stable/operator-manual/user-management/keycloak/)
- [Keycloak Docs - Keycloak](https://www.keycloak.org/)
- [Kubernetes - Default Service Endpoint](https://kubernetes.default.svc)
- [YouTube - 10분 테코톡 홍실의 Oauth 2.0](https://youtu.be/Mh3LaHmA21I?si=upaUrBNdzOZR_tBl)
