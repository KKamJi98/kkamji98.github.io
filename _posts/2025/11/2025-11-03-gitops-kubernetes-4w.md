...
**예제로 배우는 Argo CD** 책의 1장 `GitOps와 Kubernetes`를 다루겠습니다. Kubernetes가 어떻게 GitOps 개념을 도입할 수 있었는지, 선언적 API로 파일과 폴더, Git Repository에 있는 리소스를 어떻게 적용하는지 살펴봅니다.
...
- GitOps의 정의는 다양하다. 풀 리퀘스트(PR, Pull Request)를 통한 운영으로 정의하기도 하고, 개발 관행(버전 제어, 협업, 규정 준수, CI/CD)을 인프라 자동화에 적용하는 것으로 정의하기도 한다. - [GitLab - What is GitOps?](https://about.gitlab.com/topics/gitops/)
- **CNCF**의 Application Delivery TAG에 속한 **GitOps 워킹 그룹**(GitOps Working Group)이 내린 정의가 가장 중요하다. - [OpenGitOps - What is OpenGitOps?](https://opengitops.dev/)
- 이 그룹은 특정 벤더에 종속되지 않고 원칙에 입각한 GitOps 정의를 구축할 목적으로 다양한 회사 사람들로 구성돼 있다.
...
- **Declarative 선언적 구성**
  - 엔지니어가 원하는 의도와 완료된 상태를 명시하지만, 그것을 실행할 **구체적인 행동은 명시하지 않는다**.
...
- **Pulled Automatically 자동화된 배포**
  - 변경 사항이 버전 제어 시스템 VCS(Version Control System)에 반영되면 **어떠한 수동 작업도 수행하지 말아야 한다**.
...
- CNCF 내의 모든 프로젝트나 단체는 유지 관리 구조가 잘 돼 있고, 어떻게 결정하고 선정하는지를 잘 설명한다.
- 그리고 어떠한 회사도 결정에 과반수를 차지할 수 없다.
- CNCF는 커뮤니티의 참여 없이는 어떠한 결정도 내리지 않으며, 전체 커뮤니티가 프로젝트의 전반에 중요한 역할을 한다.

### 2.1. Architecture

- 쿠버네티스는 유연하게 확장 가능하기 때문에 '플랫폼 구축을 위한 플랫폼'이란 말처럼 추상적인 개념으로 접근하지 않고서는 정의하기가 어렵다.
...
- **HTTP**(HyperText Transfer Protocol) **REST API** 서버의 관점에서 Kubernetes를 보면 REST API Endpoint를 두고 상태를 데이터베이스(etcd)에 저장하는 전형적인 애플리케이션이다.
- 고가용성을 위해 여러 웹 서버 레플리카를 둔다. 쿠버네티스에서 수행되는 작업은 모두 API를 사용하기 때문에 API 서버가 멈추면 어떤 작업도 진행되지 않는다.
- 쿠버네티스의 다른 구성요소들은 **직접 통신하지 않고**, 반드시 **API 서버를 매개로 상호작용한다.**

- 클라이언트 입장에서 `curl`과 같은 도구로 API를 직접 쿼리하기는 어렵기 때문에, `kubectl`을 사용해 인증 헤더, `Request Body` 준비, `API Response` 값 파싱 같은 복잡한 처리를 간편하게 맡길 수 있다.
- `kubectl get pods`와 같은 명령을 실행할 때 **HTTPS**로 API 서버를 호출한다. 그러면 서버가 데이터베이스에서 파드 정보를 가져온 뒤 응답값을 생성해 클라이언트에게 다시 반환한다.
...
- 쿠버네티스는 많은 컨트롤러가 있다. 레플리카셋 컨트롤러는 파드의 수를 일정하게 유지한다.
- 컨트롤러는 실제 상태(live state)와 원하는 상태(desired state)가 일치하는지 관찰하고, **최종 상태에 도달하기 위해 지속적으로 조정**한다.
- 이렇게 각각의 컨트롤러는 클러스터에서 각자 맡은 부분을 관리하는 데 특화돼 있다. 직접 컨트롤러를 만들 수도 있다.
- **Argo CD** 또한 컨트롤 루프로 깃 리포지터리에 선언된 상태와 클러스터의 상태를 일치시키면서 컨트롤러로 동작한다.
...
- Deployment와 같이 복잡한 리소스의 경우 명령형 커맨드를 사용한 직접 명령 방식은 명령어에 컨테이너 이미지, 이미지 태그, 이미지 풀 정책 같은 수많은 정보를 플래그 옵션으로 지정해야 하는 불편함이 있다.
- 이러한 경우 아래와 같이 구성 파일(Configuration File)을 만들어 사용할 수도 있다. 이런 방식은 명령에 전달하는 플래그의 수가 많이 줄어들기 때문에 작업을 더 쉽게 할 수 있다.
...
- 파일을 수정하고 폴더를 apply하면 모든 변경 사항이 적용된다.
