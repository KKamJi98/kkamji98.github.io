...
tags: [kubernetes, devops, cilium, cilium-study, cilium-4w, cloudnet, gasida, routing]
comments: true
image:
...
{% include diagrams/download.html png="/assets/img/diagrams/static/kubernetes/cilium-native-routing-flow--c24770731c52b311.png" %}
...
> **TL;DR**  
> - Native Routing에서는 Cilium이 노드 바깥의 라우팅까지 책임지지 않습니다. 중간 라우터가 Pod CIDR 경로를 모르면 다른 대역의 노드에 있는 Pod로 패킷이 가지 못합니다.  
> - 실습은 서로 다른 네트워크 대역에 놓인 worker와 PodCIDR 경로가 없는 라우터에서 통신 실패를 먼저 재현합니다.  
> - 해결책은 두 가지입니다. 라우터에 static route를 직접 넣거나, BGP로 각 노드가 자기 PodCIDR을 광고하게 만듭니다.  
{: .prompt-info}
...
{% include diagrams/download.html png="/assets/img/diagrams/static/kubernetes/cilium/4w-lab-environment--7a450b3ed999f62e.png" %}
