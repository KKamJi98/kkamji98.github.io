...
{% include diagrams/static/kubernetes/helm-hook-lifecycle.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/kubernetes/helm-hook-lifecycle--f23cabdb7edeb24f.png" %}
...
> **TL;DR**  
> - DB 초기화나 스키마 마이그레이션처럼 애플리케이션보다 먼저 끝나야 하는 작업은 Helm Hook으로 lifecycle event에 묶습니다.  
> - Hook은 별도 리소스 타입이 아니라 manifest의 annotation으로 선언합니다. `helm.sh/hook`으로 시점을, `helm.sh/hook-weight`로 같은 시점 안의 순서를 정합니다.  
> - 실행 순서는 hook-weight가 낮은 `pre-install` Job들, 그다음 Deployment, 마지막으로 `post-install` Job입니다.  
{: .prompt-info}
