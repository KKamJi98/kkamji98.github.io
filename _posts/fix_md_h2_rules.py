...
SKIP_DIRS = {
    '.git', '.hg', '.svn', '.venv', 'venv', 'node_modules', '.pnpm-store',
    '.idea', '.vscode', 'dist', 'build', '__pycache__'
}
...
def process_file(path: Path, dry_run: bool, make_backup: bool, verbose: bool) -> bool:
    ...
    while i < len(lines):
        ...
        if not in_code and not in_front and is_h2(raw):
            ...
            already_has_hr = (prev is not None and prev.strip() == '---' and not just_closed_front)
    ...
    if file_changed:
        if make_backup:
            try:
                ...
                print(f'Warning: failed to write backup for {path}: {e}', file=sys.stderr)
        try:
...
def main():
    ap = argparse.ArgumentParser(description='Insert --- above every H2 (## ) header in Markdown.')
    ...
    for f in files:
        total += 1
        if process_file(f, dry_run=args.dry_run, make_backup=(not args.no_backup), verbose=args.verbose):
            changed_count += 1
