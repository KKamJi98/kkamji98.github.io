...
{% include diagrams/download.html png="/assets/img/diagrams/static/javascript/javascript-data-type-model--da87045f34b42734.png" %}
...
JavaScript는 동적 타입 언어입니다. 변수 선언에 타입을 고정하지 않으며, 변수에는 서로 다른 타입의 값을 다시 할당할 수 있습니다. 정확히 말하면 타입은 변수 자체보다 **현재 값**에 연결됩니다.
...
이 유연성은 편리하지만, 외부 입력을 받을 때는 의도하지 않은 형 변환을 만들 수 있습니다. 경계에서 `typeof`, `Array.isArray()`, 스키마 검증 등으로 값의 형태를 확인하는 습관이 필요합니다.

---

## 3. ECMAScript의 8개 언어 타입

ECMAScript 명세는 다음 8개 언어 타입을 정의합니다.
...
함수는 `typeof` 연산자에서 `"function"`을 반환하지만, ECMAScript 언어 타입 표에는 별도 타입이 아니라 호출 가능한 객체로 분류됩니다.
...
`Number`는 정수와 부동소수점을 구분하지 않는 IEEE 754 배정밀도 부동소수점 타입입니다. 정수는 `Number.MIN_SAFE_INTEGER`부터 `Number.MAX_SAFE_INTEGER`까지 정확하게 표현할 수 있습니다.
...
두 번째 결과는 0.1과 0.2가 이진 부동소수점으로 정확히 표현되지 않기 때문에 발생합니다. 금액처럼 정확한 소수 계산이 필요한 도메인에서는 정수의 최소 단위를 사용하거나 별도 정밀도 전략을 선택해야 합니다.

### 4.2. BigInt

`BigInt`는 크기 제한 없이 정수를 표현하는 원시 타입입니다. 정수 리터럴 뒤에 `n`을 붙이거나 `BigInt()`로 생성합니다. 소수는 표현할 수 없으며, `Number`와 산술 연산에서 자동으로 섞이지 않습니다.
...
`BigInt`를 JSON으로 직렬화하거나 외부 API와 주고받을 때도 별도 변환 규칙을 정해야 합니다.

---

## 5. String, Boolean, Symbol

`String`은 UTF-16 코드 단위의 순서 있는 열이며 원시 값입니다. 문자열 값을 직접 바꾸는 대신 새 문자열을 만들어 변수에 다시 할당합니다.
...
`Boolean`에는 `true`와 `false`만 있습니다. 조건문에서 다른 타입도 참 또는 거짓으로 변환될 수 있으므로, 값 자체를 비교해야 하는 경우에는 `===`를 사용합니다.

`Symbol`은 각 생성 결과가 고유한 원시 값입니다. 객체 속성 키의 이름 충돌을 피해야 할 때 사용할 수 있습니다.
...
`undefined`와 `null`은 모두 "값이 없다"는 상황에서 보이지만 의미와 생성 경로가 다릅니다.
...
`typeof null`의 결과는 역사적 호환성을 위한 예외입니다. `null`인지 확인할 때는 `value === null`을 사용합니다. 배열도 `typeof []`가 `"object"`이므로 배열 판별에는 `Array.isArray(value)`를 사용합니다.

객체는 속성의 집합이며 배열, 날짜, 정규식, 함수도 객체와 관련된 값입니다. 객체와 배열은 내용 변경이 가능하지만, `const`는 바인딩 재할당만 막습니다.
...
타입의 수와 의미는 ECMAScript 2024 명세를 기준으로 하며, 브라우저나 Node.js의 특정 API 목록은 다루지 않습니다. 이 글의 `typeof` 예시는 언어의 기본 동작을 설명하지만, 실제 입력 검증에는 애플리케이션의 데이터 형식과 오류 처리 정책이 추가로 필요합니다.
