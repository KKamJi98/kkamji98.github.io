...
같은 서브넷 안의 장비와 통신할 때는 상대 장비로 프레임을 직접 보낼 수 있습니다. 그러나 목적지 IP 주소가 로컬 네트워크 밖에 있으면 호스트는 패킷을 다음 네트워크로 전달할 라우터를 찾아야 합니다. gateway는 이 중개 지점을 폭넓게 가리키는 용어이고, default gateway는 그 가운데 일상적으로 가장 많이 만나는 하나입니다.
...
하지만 gateway를 무조건 OSI(Open Systems Interconnection) 3계층 장비라고 단정하면 범위가 너무 좁아집니다. 애플리케이션 gateway는 HTTP(Hypertext Transfer Protocol) 요청을 중계하거나 프로토콜을 변환하고, 보안 gateway는 인증과 정책 검사를 더합니다. 문서나 구성에서 gateway라는 말을 만나면 IP 라우터인지 응용 수준 중계 장치인지 먼저 확인해야 합니다.

IP 네트워크에서 라우터는 라우팅 테이블을 사용해 다음 홉과 송신 인터페이스를 선택합니다. 반면 브리지나 스위치는 일반적으로 같은 링크 계층 네트워크 안에서 MAC 주소를 기준으로 프레임을 전달합니다. 브리지나 스위치 자체를 IP default gateway와 같은 의미로 볼 수는 없습니다.
...
여기서 `/24` 같은 표기는 CIDR(Classless Inter-Domain Routing) 접두사 길이입니다. 숫자가 클수록 더 많은 앞부분 비트를 비교하므로 일반적으로 더 구체적인 경로입니다. 예를 들어 아래 라우팅 테이블에서는 `10.20.0.0/16`이 `0.0.0.0/0`보다 먼저 선택됩니다.
...
{% include diagrams/static/network/default-gateway-forwarding.html %}
{% include diagrams/download.html png="/assets/img/diagrams/static/network/default-gateway-forwarding--9dc4cf65e3410597.png" %}

1. 호스트는 라우팅 테이블에서 목적지 `198.51.100.10`에 맞는 로컬 서브넷 경로나 더 구체적인 경로를 찾습니다.
...
기본 게이트웨이가 같은 링크에 있다는 조건도 함께 봐야 합니다. 호스트는 먼저 선택한 다음 홉의 MAC 주소를 알아내야 하므로 ARP 또는 NDP가 실패하면 기본 경로가 있어도 첫 프레임을 만들 수 없습니다. 이 단계가 "라우팅 문제"와 "이웃 주소 해석 문제"를 구분하는 기준입니다.
...
기본 게이트웨이 장애는 외부 통신 실패로 보일 수 있지만 실제 원인은 하나로 정해져 있지 않습니다. 먼저 IP 주소와 CIDR 접두사, 목적지에 맞게 선택된 경로, gateway까지의 링크 계층 주소 해석, gateway 인터페이스 상태, 그 이후 경로와 방화벽 정책을 차례로 확인해야 합니다. 같은 서브넷 통신은 되는데 외부 통신만 안 되는 경우에도 기본 경로만 수정하기 전에 이 순서를 따르는 편이 안전합니다.
