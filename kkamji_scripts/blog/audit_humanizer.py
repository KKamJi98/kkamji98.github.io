...
HIGH_RULES: dict[str, re.Pattern[str]] = {
    ...
    # "이 글은 X를 다룹니다"가 구체적인 문제 서술 뒤에 오는 것은 의도한 형태라 제외한다.
    # 글의 제작 과정이나 연재 안에서의 좌표를 알리는 문장만 남긴다.
    "meta-prose": re.compile(
        r"(?:이 글의 (?:독자|범위|목표|역할|결론)|이번 (?:글|연재)에서는|도착 역량"
        r"|의도적으로 (?:제외|상세 설명하지)|이 글은 .*?시리즈의 (?:출발점|마지막|첫)"
        r"|reading-order map|시리즈 전체의 .*?역할)"
    ),
